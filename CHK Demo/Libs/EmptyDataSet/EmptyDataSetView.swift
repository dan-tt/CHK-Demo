...
public class EmptyDataSetView: UIView {
    ...
    internal func setupConstraints() {
        ...
        let centerYConstraint = NSLayoutConstraint(item: contentView, attribute: .centerY, relatedBy: .equal, toItem: self, attribute: .centerY, multiplier: 0.75, constant: 0.0)
...
//            contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:|[customView]|", options: [], metrics: nil, views: ["customView": customView]))
            ...
            if canShowImage {
                ...
                if (verticalOffset != 0) {
                    let height = frame.width > 0 ? frame.height : UIScreen.main.bounds.height
                    let imageHeight : CGFloat = (height - abs(verticalOffset))/3
                    contentView.addConstraint(NSLayoutConstraint(item: imageView, attribute: .height, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1, constant: imageHeight))
                }
            ...
            if (canShowButton) {
                ...
                button.layer.borderColor = UIColor.descColor().cgColor
